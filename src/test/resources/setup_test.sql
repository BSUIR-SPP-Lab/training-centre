...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,STRICT_ALL_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ALLOW_INVALID_DATES,ERROR_FOR_DIVISION_BY_ZERO,TRADITIONAL,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `training_service_db_test`.`student_BEFORE_INSERT` BEFORE INSERT ON `student` FOR EACH ROW
  BEGIN
    DECLARE user_count INT;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,STRICT_ALL_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ALLOW_INVALID_DATES,ERROR_FOR_DIVISION_BY_ZERO,TRADITIONAL,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `training_service_db_test`.`student_BEFORE_UPDATE` BEFORE UPDATE ON `student` FOR EACH ROW
  BEGIN
    DECLARE user_count INT;
    SET user_count = (SELECT COUNT(*) FROM `user` WHERE (NEW.`student_id` = `user`.`user_id`) AND (`user`.`role` = 'student'));
    IF user_count != 1 THEN
